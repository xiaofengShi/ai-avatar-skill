...
仓库包含两类示例：六组使用 AI 生成虚构人物的肖像，以及一组用户明确授权用于项目示例展示的真实旅行照片与插画（由 README 迁至项目主页）。授权限于本次旅行案例，没有扩展到其他私人照片、真实证件照或早期对话的原始风格图。
...
- `examples/developer/preview-screenshot.png`：2026-09-22 从 `examples/developer/preview.html` 截取的历史预览截图。因分辨率只有 524×395，不再用于页面展示；当前页面直接用高清头像渲染圆形裁切。
...
- `examples/studio-id-photo/source.png` 与 `portrait.png`：2026-09-24 由 Codex 内置图像工具分别生成虚构中国成年女性的普通源照片，以及以该照片为唯一人物参考的一张精致影棚证件照风格肖像。用户提出“海马体证件照”方向，并指定[海马体官网](https://www.haimati.cn/#/)供今后参考；本案例仅借用精致影棚照的视觉语言，未使用海马体样片、标志或服务，与该品牌无关联。成片用于简历／员工资料场景的演示，未核验任何正式证件或考试报名规格。

文件哈希、生成入口、参考图顺序和实际指令记录在 [manifest.json](examples/manifest.json)。工具没有公开的模型标识与 seed 记为 `null`。图像不会因记录相同 prompt 而保证逐像素重现。

## 项目标志

`assets/logo.png` 为 Codex 内置图像工具生成的项目标志，以肖像取景框、人物头部和字母 A 组合，沿用陶土红配色。首版因视觉效果未被用户接受，保留原文件，当前页面不再使用。

`assets/logo-v2.png` 为重新生成的侧脸轮廓与开放取景边框标志，用于 README、主页页头及浏览器图标；未引用真人照片或第三方标志。`assets/wordmark.svg` 与 `assets/homepage-link.svg` 是文字排版资源，控制 GitHub 标题和入口的字号及对齐。
