...
The [English ChatGPT prompt](https://github.com/xiaofengShi/ai-avatar-skill/blob/main/prompts/chatgpt.en.md) was translated and generated on 2026-09-23. The Chinese observations above are separate evidence. An earlier logged-out attempt sent only an excerpt of the English planning rules; the browser timed out before a reply could be verified.

On 2026-09-23, a new normal conversation in **signed-in Chrome ChatGPT** received the complete English planning-stage block plus “I want to turn a photo into a nice avatar.” The sent text matched the generated block plus that sentence (SHA-256 `981c1b06daab9bf7de51976bb17aead3762b2fbc82518c4edd109bc75e4ebe94`). ChatGPT replied only, “Where will this image primarily be used?” No image was shown in that turn.

In the same conversation, the assistant uploaded only the repository's public fictional [source.png](examples/developer/source.png) (SHA-256 `8429e5704d0ec7aa9f1ee2ef4fd040d2a3b6f3c7789d3eb6843467e03a7197db`) and sent: “For my GitHub and Hugging Face profile avatars. Keep the glasses; design everything else yourself. The attached source.png is the person and identity reference. No style reference is attached.” ChatGPT returned a text plan for a square head-and-shoulders portrait, dark top, muted cool-gray background, and semi-realistic hand-drawn style. It explicitly retained the glasses, salt-and-pepper hair, and facial hair. It did not generate an image or ask for a photography-parameter checklist in this turn.

The exact English generation-stage message then produced one visible image. The downloaded 1254 × 1254 PNG has SHA-256 `62eb897cd70c36d1bef8a0fbd669ea14141dc18ccb4df685b55b701e64de10b2`; it is not included in the repository. In the 60% browser view and downloaded full image, the glasses, graying hair, and facial hair remain visible; clothing is a dark knit top against a light blue-gray background. The face still reads largely as a photograph with some sketch-like texture, so the proposed hand-drawn style was only partly realized. No obvious head/neck/shoulder mismatch was visible. ChatGPT delivered only the image, without a visible account of any small-size or circular-crop review; this trial did not perform a circular-crop check. The page did not expose the internal image prompt, model version, or seed. This single fictional-person result does not establish identity fidelity or stylistic consistency. Editing and two-image style-reference behavior remain untested. The exact messages and observations are in the [English trial record](tests/chatgpt-en-observations.json).
...
For this release, the GitHub Pages build reported success for the published commit, and the live English and Chinese homepages plus both new English pages matched the committed HTML byte for byte. A clean temporary installation from the public GitHub skill path succeeded with the Codex skill installer, and the installed preview script generated an embedded-image HTML preview from the public fictional avatar. The first installer attempt failed because this Mac's Python did not have a usable default certificate chain; setting `SSL_CERT_FILE` to the already installed `certifi` bundle allowed the same installer to complete. This is an environment-specific installation check, not a test of skill invocation, aesthetic quality, or image generation. The new English pages were checked for existing local links and anchors. A Chrome desktop view of the English gallery showed the hero and first two case images loaded in the two-column layout; this release was not visually checked at a mobile width.
